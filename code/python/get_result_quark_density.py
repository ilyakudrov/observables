...
def get_quark_density(data):
    x = np.array(data['quark_density'].to_numpy())
    x = np.array([x])

    field, err = stat.jackknife_var_numba(x, trivial)

    return pd.Series([field, err], index=['quark_density', 'err'])
...
def read_quark_density(path):
    data = []
    with open(path) as file:
        while line := file.readline():
            if(line[0] != '#'):
                data.append(float(line))
    return pd.DataFrame({'quark_density': data})
...
for matrix_type, beta, conf_size, mu, additional_parameters in itertools.product(*iter_arrays):
    ...
    for chain in chains:
        for i in range(0, conf_max + 1):
            if copy_single:
                file_path = f'{base_path}/chiral_condensate/{theory_type}/{conf_type}/{conf_size}/{beta}/{mu}/{matrix_type}/{additional_parameters}/{chain}/QuarkDens_{i:04}.txt'
                # print(file_path)
                if (os.path.isfile(file_path)):
                    data.append(read_quark_density(file_path))
                    data[-1]["conf"] = i
                    data[-1]["copy"] = copies
            else:
    ...
    if len(data) == 0:
        ...
        if is_binning:
            ...
            df1 = df.groupby('copy').apply(get_quark_density, include_groups=False).reset_index(level='copy')
        if copy_single:
        ...
        if is_binning:
            ...
            binning = ''
        path_output = f"../../result/{binning}/quark_density/{theory_type}/{conf_type}/{conf_size}/{beta}/{mu}/{additional_parameters}"
        try:
            os.makedirs(f'{path_output}')
        except:
            pass
        df1.to_csv(
            f"{path_output}/quark_density_{matrix_type}.csv", index=False)
