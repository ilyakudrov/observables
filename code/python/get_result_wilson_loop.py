...
for matrix_type, smeared, beta, conf_size, mu, additional_parameters in itertools.product(*iter_arrays):
    ...
    if len(data) == 0:
        ...
        # df1 = df1[names_out]
        path_output = f"../../result/wilson_loop/{representation}/{axis}/{theory_type}/{conf_type}/{conf_size}/{beta}/{mu}/{smeared}/{additional_parameters}"
        try:
            os.makedirs(f'{path_output}/{dir_name}')
        except:
            pass
        df1.to_csv(
            f"{path_output}/{dir_name}/wilson_loop_{matrix_type}.csv", index=False)
