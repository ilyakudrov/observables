...
for beta, decomposition_plaket, decomposition_wilson, conf_size, mu, additional_parameters, smearing in itertools.product(*iter_arrays):
    ...
    for chain in chains:
        for i in range(conf_max + 1):
            file_path_electric = f'../../data/flux_tube_wilson/{theory_type}/{conf_type}/{conf_size}/{beta}/{mu}/{decomposition_plaket}-{decomposition_wilson}/{smearing}/{chain}/{direction}/electric_{i:04}'
            ...
            if(os.path.isfile(file_path_electric) & os.path.isfile(file_path_magnetic)):
                data_electric.append(pd.read_csv(file_path_electric, header=0, names=[
                                     'T', 'R', flux_coord, 'correlator electric', 'wilson loop', 'plaket electric']))
                if shift:
    ...
    path_output = f"../../result/flux_tube_wilson/{theory_type}/{conf_type}/{conf_size}/{beta}/{mu}/{smearing}/{direction}"
