...
def read_data_condensates(filename):
    data_chiral = []
    data_diquark = []
    conf = []
    stoch_vec = []
    ...
    with open(filename) as file:
        while line := file.readline():
            if line.startswith('# IPARAM'):
                conf_num = int(line.split(' ')[3][5:])
                count = 1
            if line[0] != '#':
                conf.append(conf_num)
                data_chiral.append(float(line.split('\t')[0]))
                data_diquark.append(float(line.split('\t')[1]))
                stoch_vec.append(count)
                count += 1
    df = pd.DataFrame({'conf': conf, 'chiral_condensate': data_chiral, 'diquark_condensate': data_diquark})
    df = df.astype({'conf': int, 'chiral_condensate': float, 'diquark_condensate': float})
    return df
...
conf_type = "qc2dstag"
conf_sizes = ['Nt10_Ns32', 'Nt12_Ns32', 'Nt14_Ns32', 'Nt16_Ns32', 'Nt18_Ns32', 'Nt20_Ns32']
# conf_sizes = ['32^3x16']
theory_type = 'su2'
betas = ['beta1.75']
copies = 0
...
additional_parameters_arr = ['rho0.15_ma0.0075_lambda0.00075']
...
#mu1 = ['mu0.00', 'mu0.05']
mu1 = ['mua_re0.03', 'mua_re0.07', 'mua_re0.12']
# mu1 = ['mu0.15']
# mu1 = ['/']
#chains = ["/"]
# mu1 = ['mu0.05',
...
# mu1 = ['mu0.40']
# chains = ['s1', 's2']
chains = ['', '_s1', '_s2', '_s3', '_s4']

# adjoint_fix = True
adjoint_fix = False

# base_path = "../../data"
base_path = "../../data/vladimir_data/condensates"
...
for matrix_type, beta, conf_size, mu, additional_parameters in itertools.product(*iter_arrays):
    ...
    data = []
    for chain in chains:
        file_path = f'{base_path}/{conf_size}/{beta}/{additional_parameters}/{mu}{chain}/Condensates.txt'
        if (os.path.isfile(file_path)):
            data.append(read_data_condensates(file_path))
            data[-1]['copy'] = 0
            if chain == '':
                data[-1] = data[-1][data[-1]['conf'] > 200]
    if len(data) == 0:
        ...
        if is_binning:
            ...
            binning = ''
        path_output = f"../../result/{binning}/condensates_vladimir_data/{theory_type}/{conf_type}/{conf_size}/{beta}/{mu}/{additional_parameters}"
        try:
