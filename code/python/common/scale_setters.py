...
class ExtendedSymanzikScaleSetter(SymanzikScaleSetter):
    """Extended range scale setter for Symanzik action gluodynamics.

    Applicable for betas within [3.85, 5.5]. Collected data are for betas
    within [3.85, 5.3].
    ...
    def get_beta_domain(self) -> domains.GeometricRange:
        """See base class."""
        return domains.GeometricRange((3.85, 5.5))

    def get_spacing_in_fm(self, beta: Sequence[float]) -> np.ndarray:
        """See base class.

        Applicable for beta within [3.85, 5.5].
        """
        return np.array([self._get_spacing_in_fm(_beta) for _beta in beta])
    ...
    def _get_spacing_in_fm(self, beta: float) -> float:
        ...
        if 3.85 <= beta <= 5.5:
            ...
            raise ValueError(f"beta has to be within [3.85, 5.5] interval "
                             f"(received beta={beta}).")
