...
int main(int argc, char *argv[]) {
  ...
  std::map<std::tuple<int, double>, std::vector<double>> data;
  ...
  if (!data.empty()) {
    ...
    std::map<std::tuple<int, double>, std::tuple<double, double>> result;
    for (auto pair : data) {
      jackknife_data = do_jackknife(pair.second, bin_borders);
      result[pair.first] = get_aver(jackknife_data);
    }
