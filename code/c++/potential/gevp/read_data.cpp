...
// n is the number of copies among first which the best copy is being picked
void read_csv_functional_best_of_n(
    std::string file_path,
    std::map<std::tuple<std::string, int, int>, double> &functional,
    std::string chain, int conf_num, int n) {
  std::ifstream file_stream(file_path);
  std::string line;
  std::vector<std::string> parsed_line;
  std::getline(file_stream, line);
  int best_copy_num = -1;
  int copy_num = -1;
  double best_copy_functional = 0;
  while (std::getline(file_stream, line) && copy_num < n - 1) {
    parsed_line = parse_line(line);
    copy_num = std::stoi(parsed_line[0]);
    if (std::stod(parsed_line[1]) > best_copy_functional) {
      best_copy_functional = std::stod(parsed_line[1]);
      best_copy_num = copy_num;
    }
  }
  functional[{chain, conf_num, best_copy_num}] = best_copy_functional;
}
