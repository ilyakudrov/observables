...
do_jackknife(const std::vector<std::vector<double>> &data,
...
#pragma omp parallel for reduction(+ : sum) reduction(+ : sum_squared)         \
    shared(data)
  for (int j = 0; j < n; j++) {
    sum += data[j];
    sum_squared += data[j] * data[j];
  }
  std::vector<std::vector<double>> data_jackknife(
      2, std::vector<double>(bin_borders.size() - 1));
  int l = bin_borders.size() - 1;
#pragma omp parallel for shared(data_jackknife, bin_borders)                   \
    firstprivate(sum, sum_squared)
  for (int j = 0; j < l; j++) {
    data_jackknife[0][j] = sum;
    data_jackknife[1][j] = sum_squared;
    for (int k = bin_borders[j]; k < bin_borders[j + 1]; k++) {
      data_jackknife[0][j] -= data[k];
      data_jackknife[1][j] -= data[k] * data[k];
    }
    data_jackknife[0][j] =
        data_jackknife[0][j] / (n - bin_borders[j + 1] + bin_borders[j]);
    data_jackknife[1][j] =
        data_jackknife[1][j] / (n - bin_borders[j + 1] + bin_borders[j]) -
        data_jackknife[0][j] * data_jackknife[0][j];
  }
  return data_jackknife;
}