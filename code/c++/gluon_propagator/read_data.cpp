...
void read_momenta_from_csv(std::string file_path,
                           std::set<std::tuple<int, int, int, int>> &momenta) {
  std::ifstream file_stream(file_path);
  std::string line;
  std::vector<std::string> parsed_line;
  std::getline(file_stream, line);
  while (std::getline(file_stream, line)) {
    parsed_line = parse_line(line);
    momenta.insert({std::stoi(parsed_line[0]), std::stoi(parsed_line[1]),
                    std::stoi(parsed_line[2]), std::stoi(parsed_line[3])});
  }
}
...
void read_csv_single_momentum(
    std::tuple<int, int, int, int> &momentum, std::string file_path,
  ...
  while (std::getline(file_stream, line)) {
    parsed_line = parse_line(line);
    if (std::stoi(parsed_line[0]) == std::get<0>(momentum) &&
        std::stoi(parsed_line[1]) == std::get<1>(momentum) &&
        std::stoi(parsed_line[2]) == std::get<2>(momentum) &&
        std::stoi(parsed_line[3]) == std::get<3>(momentum)) {
